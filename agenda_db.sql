...
ALTER TABLE usuario ADD COLUMN imagenId INT(11);
ALTER TABLE usuario ADD FOREIGN KEY (imagenId) REFERENCES imagen(imagenId);



CREATE TABLE direcciones (
    direccionId INT(11) NOT NULL AUTO_INCREMENT, -- ID único para cada dirección
    contactoId INT(11) NOT NULL, -- Relación con la tabla contacto
    direccion VARCHAR(255) NOT NULL, -- La dirección propiamente dicha
    PRIMARY KEY (direccionId), -- Clave primaria
    KEY IX_Direcciones_contactoId (contactoId), -- Índice para optimizar búsquedas
    CONSTRAINT FK_Direcciones_Contacto_contactoId FOREIGN KEY (contactoId) REFERENCES contacto (contactoId) ON DELETE CASCADE
) ENGINE=INNODB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4;
